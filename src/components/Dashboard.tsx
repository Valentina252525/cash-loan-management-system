...
export default function Dashboard() {
  const metrics = [
    ...
    { title: 'Revenue (This Month)', value: 'TZS 12.4M', color: 'bg-yellow-400 text-black' },
    { title: 'Pending Approvals', value: '8', color: 'bg-red-600 text-white' },
  ];

  const recentLoans = [
    { customer: 'John Doe', amount: 'TZS 500,000', status: 'Approved', date: 'Oct 12, 2025' },
    { customer: 'Jane Smith', amount: 'TZS 800,000', status: 'Pending', date: 'Oct 14, 2025' },
  ];
  ...
  return (
    <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {metrics.map((m) => (
          <Card key={m.title} className={`${m.color} text-white shadow-lg hover:shadow-xl transition-shadow`}>
      ...
      <Card className="shadow-md">
        <CardHeader>
          <CardTitle className="text-xl font-semibold text-gray-900">Recent Loan Activity</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {recentLoans.map((loan) => (
                <TableRow key={loan.customer} className="hover:bg-gray-50">
                  ...
                  <TableCell className="text-gray-600">{loan.date}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
